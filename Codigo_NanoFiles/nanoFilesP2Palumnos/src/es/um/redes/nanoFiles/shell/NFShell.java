...
public class NFShell {
	...
	private String[] readGeneralCommandFromStdIn() {
		...
		while (true) {
			switch(NFController.getCurrentState()){
			case NFController.OFFLINE:
				System.out.print("([OFFLINE] nanoFiles@" + NanoFiles.sharedDirname + ") ");
				break;
			case NFController.JOINED:
				System.out.print("([JOINED] nanoFiles@" + NanoFiles.sharedDirname + ") ");
				break;
			case NFController.SERVE:
				System.out.print("([SERVE] nanoFiles@" + NanoFiles.sharedDirname + ") ");
				break;
			default:
			}
