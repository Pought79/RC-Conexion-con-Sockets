...
public class DirectoryConnector {
	...
	private static final int FAIL_ERROR = 1;
	/**
	...
	private byte[] sendAndReceiveDatagrams(byte[] requestData) {
		...
		while (i < MAX_NUMBER_OF_ATTEMPTS) {
			try {
				...
				System.out.println("Ha fallado en el envio y se ha tenido que cerrar el programa");
				System.exit(FAIL_ERROR);
		...
		System.arraycopy(paqueteRespuesta.getData(), 0, response, 0, paqueteRespuesta.getLength()); // Se encarga de
																									// copiar solo los
																									// datos
	...
	public boolean testSendAndReceive() {
		 ...
		 */
		String cad = "ping";
		byte[] bytesCad = cad.getBytes();
		byte[] prueba = sendAndReceiveDatagrams(bytesCad);
		String cadDevuelta = new String(prueba);
		if (cadDevuelta.startsWith("pingok")) {
			return true;
		}
		return false;
	}
	...
	public boolean pingDirectoryRaw() {
		 ...
		 */
		String id_protocolo = new String(NanoFiles.PROTOCOL_ID);
		String cadena_mensaje = "ping&" + id_protocolo;
		byte[] mensaje_enviar = cadena_mensaje.getBytes();
		byte[] mensaje_recibir = sendAndReceiveDatagrams(mensaje_enviar);
		String cadena_recibida = new String(mensaje_recibir);
		if (cadena_recibida.equals("welcome")) {
	...
	public boolean pingDirectory() {
		...
		String mensaje_enviar = mensaje.toString();
		DatagramPacket datagrama_enviar = new DatagramPacket(mensaje_enviar.getBytes(),
				mensaje_enviar.getBytes().length, directoryAddress);
		if (NanoFiles.nano_debug_2)
			System.out.println("* [PING] Sending PING message from client...");
		byte[] mensaje_recibir = sendAndReceiveDatagrams(datagrama_enviar.getData());
		String texto_devuelto = new String(mensaje_recibir);
		DirMessage mensaje_recibido = DirMessage.fromString(texto_devuelto);
		if (mensaje_recibido.getOperation().equals(DirMessageOps.OPERATION_PING_SUCCESS)) {
			success = true;
		}
		return success;
	...
	public boolean registerFileServer(int serverPort, FileInfo[] files) {
		...
		for (int i = 0; i < files.length; i++) {
			...
			nombres.append(files[i].fileName);
			tamanos.append(String.valueOf(files[i].fileSize));
			hashes.append(files[i].fileHash);
			primeraVez = false;
		}
		mensaje.setFileNames(nombres);
		mensaje.setFileSizes(tamanos);
		mensaje.setFileHashes(hashes);
		String mensaje_enviar = mensaje.toString();
		DatagramPacket datagrama_enviar = new DatagramPacket(mensaje_enviar.getBytes(),
				mensaje_enviar.getBytes().length, directoryAddress);
		if (NanoFiles.nano_debug_2)
			System.out.println("* [SERVE] Sending SERVE message from client...");
		byte[] mensaje_recibir = sendAndReceiveDatagrams(datagrama_enviar.getData());
		String texto_devuelto = new String(mensaje_recibir);
		DirMessage mensaje_recibido = DirMessage.fromString(texto_devuelto);
		if (mensaje_recibido.getOperation().equals(DirMessageOps.OPERATION_SERVE_SUCCESS)) {
			success = true;
		}
	...
	public FileInfo[] getFileList() {
		...
		DirMessage mensaje = new DirMessage(DirMessageOps.OPERATION_GETFILELIST);
		String mensaje_enviar = mensaje.toString();
		DatagramPacket datagrama_enviar = new DatagramPacket(mensaje_enviar.getBytes(),
				mensaje_enviar.getBytes().length, directoryAddress);
		if (NanoFiles.nano_debug_2)
			System.out.println("* [GETFILELIST] Sending GETFILELIST message from client...");
		byte[] mensaje_recibir = sendAndReceiveDatagrams(datagrama_enviar.getData());
		String texto_devuelto = new String(mensaje_recibir);
		DirMessage mensaje_recibido = DirMessage.fromString(texto_devuelto);
		if (mensaje_recibido.getOperation().equals(DirMessageOps.OPERATION_FILELIST)) {
			if (!mensaje_recibido.getFileNames().isEmpty()) {
				String[] nombres = mensaje_recibido.getFileNames().toString().split(", ");
				String[] tamanos = mensaje_recibido.getFileSizes().toString().split(", ");
				String[] hashes = mensaje_recibido.getFileHashes().toString().split(", ");
				filelist = new FileInfo[nombres.length]; // Como los nombres, los tamaños y los hashes tendrán los
															// mismos elementos,
															// elegimos cualquiera como tamaño del array FileInfo
				for (int i = 0; i < filelist.length; i++) {
					filelist[i] = new FileInfo();
					filelist[i].fileName = nombres[i];
					filelist[i].fileSize = Long.parseLong(tamanos[i]);
					filelist[i].fileHash = hashes[i];
				}
			}
		}
		return filelist;
	...
	public InetSocketAddress[] getServersSharingThisFile(String filenameSubstring) {
		...
		DirMessage mensaje = new DirMessage(DirMessageOps.OPERATION_DOWNLOAD_REQUEST);
		mensaje.setFileNameSubstring(filenameSubstring);
		String mensaje_enviar = mensaje.toString();
		DatagramPacket datagrama_enviar = new DatagramPacket(mensaje_enviar.getBytes(),
				mensaje_enviar.getBytes().length, directoryAddress);
		if (NanoFiles.nano_debug_2)
			System.out.println("* [DOWNLOAD] Sending DOWNLOAD message from client...");
		byte[] mensaje_recibir = sendAndReceiveDatagrams(datagrama_enviar.getData());
		String texto_devuelto = new String(mensaje_recibir);
		DirMessage mensaje_recibido = DirMessage.fromString(texto_devuelto);
		if (mensaje_recibido.getOperation().equals(DirMessageOps.OPERATION_DOWNLOAD)) {
			String[] servidores = mensaje_recibido.getServers().toString().split(",");
			serversList = new InetSocketAddress[servidores.length];
			for (int i = 0; i < servidores.length; i++) {
				try {
					String servidor = servidores[i].trim();
					String[] parts = servidor.split(":");
					String host = parts[0];
					int port = Integer.parseInt(parts[1]);
					serversList[i] = new InetSocketAddress(InetAddress.getByName(host), port);
				} catch (Exception e) {
					if (!NanoFiles.nano_debug_0)
						System.err.println("* [ERROR] Error converting String to InetSocketAddress: " + servidores[i]);
					e.printStackTrace();
				}
			}
		}
	...
	/**
	 * Método para darse de baja como servidor de ficheros dado un puerto
	 * específico.
	 * 
	 * @param serverPort El puerto TCP en el que este peer sirve ficheros a otros.
	 * @return Verdadero si el directorio tiene registrado a este peer como servidor
	...
	public boolean unregisterFileServer(int serverPort) {
		boolean success = false;
		DirMessage mensaje = new DirMessage(DirMessageOps.OPERATION_UNPUBLISH_FILES);
		mensaje.setPort(serverPort);
		String mensaje_enviar = mensaje.toString();
		DatagramPacket datagrama_enviar = new DatagramPacket(mensaje_enviar.getBytes(),
				mensaje_enviar.getBytes().length, directoryAddress);
		if (NanoFiles.nano_debug_2)
			System.out.println("* [UNPUBLISH] Sending UNPUBLISH_FILES message from client...");
		byte[] mensaje_recibir = sendAndReceiveDatagrams(datagrama_enviar.getData());
		String texto_devuelto = new String(mensaje_recibir);
		DirMessage mensaje_recibido = DirMessage.fromString(texto_devuelto);
		if (mensaje_recibido.getOperation().equals(DirMessageOps.OPERATION_UNPUBLISH_FILES_SUCCESS)) {
			success = true;
		}
		return success;
	}
