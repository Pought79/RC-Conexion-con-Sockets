...
public class NFController {
	...
	public static final byte SERVE = 2;
	...
	public void processCommand() {
		...
		case NFCommands.COM_SERVE:
			...
			break;
		case NFCommands.COM_STOPSERVER:
			/*
	...
	private boolean canProcessCommandInCurrentState() {
		...
		switch (currentCommand) { // Si filelist, download o myfiles son ejecutados sin un ping previo, saltará el
									// mensaje de que no se procesar el comando hasta que se haga un ping
		case NFCommands.COM_FILELIST:
		case NFCommands.COM_DOWNLOAD:
		case NFCommands.COM_SERVE:
		case NFCommands.COM_QUIT:
		case NFCommands.COM_STOPSERVER: {
			if (currentState == OFFLINE) {
				System.out.println(
						"* To perform this operation, you must first verify using the 'ping' command that the client protocol is compatible with the directory protocol");
				commandAllowed = false;
				break;
			} else if (currentCommand == NFCommands.COM_STOPSERVER && currentState != SERVE) {
				System.out.println("To perform this operation, the server must first execute the 'serve' command");
				commandAllowed = false;
				break;
			}
			break;
		...
		case NFCommands.COM_UPLOAD: {
			commandAllowed = (currentState == SERVE);
			if (!commandAllowed) {
				System.out.println("To perform this operation, the server must first execute the 'serve' command");
			}
			break;
		...
		default:
			System.err.println("* [Error] Invalid command. Please check your input and try again.");
			commandAllowed = false;
		}
	...
	private void updateCurrentState(boolean success) {
		...
		switch (currentCommand) {
		case NFCommands.COM_PING: // Comprobamos la conexión entre cliente y servidor
			if (currentState != SERVE)
				currentState = JOINED;
			break;
		case NFCommands.COM_SERVE: // Establecemos al cliente como un servidor TCP
			currentState = SERVE;
			break;
		case NFCommands.COM_STOPSERVER:
			if (currentState != OFFLINE)
				currentState = JOINED; // Volvemos al estado que va antes de 'serve', para así poder ejecutarlo
			break;
		default:
		}
